...
const [activeChatId, setActiveChatId] = useState<string>(() => crypto.randomUUID());
  ...
  const [messages, setMessages] = useState<Message[]>([
    { 
      id: 'welcome', 
      role: 'ai', 
      content: "Hello! I am your TenderAI assistant. \n\nPlease upload a Tender or RFP document in the sidebar so I can analyze it for you.",
      timestamp: new Date()
    }
  ]);
  ...
  // --- CHAT NAVIGATION ---
  const handleNewChat = () => {
    setActiveChatId(crypto.randomUUID()); // Generate a new room ID
    setMessages([
      { id: 'welcome', role: 'ai', content: "Starting a new session. Please upload a new document or start chatting.", timestamp: new Date() }
    ]); 
  ...
  const handleSelectChat = (chatId: string) => {
    setActiveChatId(chatId); // Change the active room
    
    // Clear the current messages and show a loading state
    setMessages([
      { id: 'loading', role: 'ai', content: "Loading previous conversation...", timestamp: new Date() }
    ]);
    
    // Note: Soon, we will add a fetch() call here to load the LangGraph messages from Python!
  };
  ...
  useEffect(() => {
   // 1. Grab the base URL from the environment file
    // In production, this will automatically swap to your real domain!
    const baseUrl = import.meta.env.VITE_API_BASE_URL;

    // 2. Build the dynamic connection
    const connection = new signalR.HubConnectionBuilder()
      .withUrl(`${baseUrl}/hubs/document`) // Dynamic string interpolation
      .withAutomaticReconnect(jitterRetryPolicy)
      .build();

    // 2. Tell React what to do when it hears the "DocumentProcessed" shout
   connection.on("DocumentProcessed", (data) => {
    console.log("⚡ SignalR Message Received:", data);

    // Assuming the C# worker sends { status: "Completed", summary: "...", fileName: "..." }
    if (data.status === 'Completed') {
        // 1. Update the Sidebar UI
        setUploadStatus('success');
        setStatusMessage('Analysis Complete!');
        setIsThinking(false); // Turn off the typing indicator if it's on

        // 2. Add the AI's summary directly into the chat!
  ...
  useEffect(() => {
    const fetchSidebar = async () => {
      try {
        // Calling the C# endpoint we just built
  ...
  if (!connectionId) {
    ...
    formData.append('UserId', 'demo-user-01'); // Using a fixed ID for the demo
    formData.append('ConnectionId',connectionId);
    formData.append('ChatId', activeChatId);

    try {
      // The Proxy in vite.config.ts forwards this to your C# API
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // 4. Show AI Message
      // Note: Assuming your API returns { answer: "..." }
